...
func TestDb_DeleteDoc(t *testing.T) {
	// Хранилище - память
	storage := mem.New()
	// Поисковая структура - бинарное дерево
	searcher := btree.New()
	...
	if _, found := db.Find(id); found == false {
		t.Fatalf("Документ не найден: %d", id)
	}

	db.DeleteDoc(id)
	if doc, found := db.Find(id); found == true {
		t.Fatalf("Документ найден: %d, %v", id, doc)
	}
}
